...
<html>
  <head>
	...
	<script type="text/javascript">
		function updateStatus(accountNo){
          	location.href='<c:url value="financial_accountMgr?act=forUpdate"/>&accno='+accountNo;      
       	}
  ...
  <body>
    <div id="wrapper">
	     <div id="qryarea">
		     <form action="<c:url value="/financial_accountMgr?act=loadPagedFundAccountsStatus"/>" method="post">
		        <span>账号:</span>
		        <input type="text" name="qryaccno" size="20"/>
		        <span>状态:</span>
		         <select name="qryaccstatus">
		           <option value="">== 请选择 == </option>
		           <option  value="a" <c:if test="${param.qryfundstatus=='a'}">selected</c:if>> 可用</option>
		           <option  value="b" <c:if test="${param.qryfundstatus=='b'}">selected</c:if>> 冻结</option>
		         </select>
	     ...
	     <div>
	       ...
	       <caption>资金账户状态</caption>
	          ...
	          <c:forEach var="financial_account" items="${page.pageContent}">
		            ...
		            <td>
		            <button onclick="updateStatus(${financial_account.acc_no});">修改</button> 
